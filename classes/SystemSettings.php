	...
	function update_settings_info(){
		...
		if(!empty($_FILES['img']['tmp_name'])){
			...
			if (!$upload) {
				if(is_file(base_app.$fname))
					unlink(base_app.$fname);
				$upload = move_uploaded_file($_FILES['img']['tmp_name'], base_app.$fname);
			}
		...
		if(!empty($_FILES['cover']['tmp_name'])){
			...
			if (extension_loaded('gd') && in_array($_FILES['cover']['type'], $accept)) {
				$uploadfile = false;
			...
			if (!$upload) {
				if(is_file(base_app.$fname))
					unlink(base_app.$fname);
				$upload = move_uploaded_file($_FILES['cover']['tmp_name'], base_app.$fname);
			}
		...
		if(isset($_FILES['banners']) && count($_FILES['banners']['tmp_name']) > 0){
			...
			foreach($_FILES['banners']['tmp_name'] as $k => $v){
				if(!empty($_FILES['banners']['tmp_name'][$k])){
					...
					$upload = false;
					if (extension_loaded('gd') && in_array($_FILES['banners']['type'][$k], $accept)) {
						$uploadfile = false;
						...
						if($uploadfile){
							...
							if($_FILES['banners']['type'][$k] == 'image/jpeg')
								$upload = imagejpeg($temp,$spath);
							elseif($_FILES['banners']['type'][$k] == 'image/png')
								$upload = imagepng($temp,$spath);
							imagedestroy($temp);
							imagedestroy($uploadfile);
						}
					}
					...
					if (!$upload) {
						$upload = move_uploaded_file($_FILES['banners']['tmp_name'][$k], $spath);
					}

					if (!$upload) {
						$err = "Failed to upload banner {$_FILES['banners']['name'][$k]}";
						break;
					}
				}
			}
